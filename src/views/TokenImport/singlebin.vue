...
const uploadBin = (binFile: File) => {
  tQueue.add(async () => {
    ...
    reader.onload = async (e) => {
      ...
      message.success("Token 已暂存, 提交时由服务端处理");
      roleList.value.push({
        id: tokenId,
        name: roleName,
...
const handleImport = async () => {
  ...
  try {
    ...
    for (const role of roleList.value) {
      const binBase = role.bin
        ? btoa(String.fromCharCode(...new Uint8Array(role.bin)))
        : '';
      const result = await tokenStore.importToken({
        method: 'manual',
        name: role.name,
        bin: binBase,
        server: role.server,
    ...
    message.success(`成功导入 ${imported} 个 Token`);
  } catch (e: any) {
