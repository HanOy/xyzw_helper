...
| 设置持久化 key | 后端 settings 表; nickname 维度 | `daily-settings:{nickname}` / `dream-items:{nickname}` / `batchSettings` / `task-templates`; nickname = tokens.name (用户导入时设的名字, 跨 token 重导稳定, 同一游戏账号自动复用); `batchSettings` 是全局; 前端一律走 settingsStore, 禁止 localStorage 存配置 |
